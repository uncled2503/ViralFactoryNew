...
export const DashboardOverview: React.FC = () => {
  ...
  // Wizard Flow for Template & Mass Rendering — the real "template → position →
  // batch → render" pipeline lives in NewProjectWizard; this screen only decides
  // when it's open and which entry point (project vs. mass render) opened it.
  const [isFlowOpen, setIsFlowOpen] = useState(false);
  const [flowType, setFlowType] = useState<'project' | 'mass_render'>('project');

  const openWizard = (type: 'project' | 'mass_render') => {
    setFlowType(type);
    setIsFlowOpen(true);
  };

  // Onboarding checklist — real completion state, no fake "recommended pipeline"
  // text block. Each item is clickable and jumps straight to the relevant screen.
  const onboardingSteps = [
    {
      id: 'template',
      label: 'Enviar um template',
      description: 'O background que servirá de estrutura para os seus vídeos.',
    ...
    },
    {
      ...
      description: 'Posicione a área do vídeo e envie o lote a processar.',
    ...
    {
      id: 'render',
      label: 'Rodar uma renderização',
      description: 'Gere seu primeiro vídeo finalizado.',
      done: renderingTasks.some((t) => t.status === 'completed'),
      icon: Film,
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3.5 pt-2">
              ...
              <button
                onClick={() => openWizard('mass_render')}
          ...
          {/* Quick Access — real shortcuts, no fake "simulate upload" theater */}
          <div className="lg:col-span-5 grid grid-cols-2 gap-3">
            <button
              onClick={() => setActiveTab('templates')}
              className="p-4 rounded-xl border border-gray-800 bg-gray-950/40 hover:border-indigo-500/40 hover:bg-indigo-950/5 transition-all duration-300 text-left cursor-pointer flex flex-col justify-between h-24"
            >
              <Layers className="w-4 h-4 text-indigo-400" />
              <div>
                <p className="text-xs font-semibold text-gray-200">Templates</p>
                <p className="text-[10px] text-gray-500">{templates.length} disponíveis</p>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div>
                <p className="text-xs font-semibold text-gray-200">Projetos</p>
                <p className="text-[10px] text-gray-500">{projects.length} criados</p>
              </div>
            </button>
            <button
              onClick={() => setActiveTab('renderings')}
            ...
            >
              ...
              <div>
                <p className="text-xs font-semibold text-gray-200">Renderizações</p>
                <p className="text-[10px] text-gray-500">{activeRenders.length} em andamento</p>
              </div>
            </button>
          </div>
        </div>
      ...
      {/* 2. ONBOARDING CHECKLIST — real progress, hidden once every step is done */}
      {!onboardingComplete && (
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 rounded-xl border border-gray-900 bg-gray-950/40">
          ...
          {onboardingSteps.map((step, i) => {
            ...
            return (
              <button
                ...
                disabled={step.done}
                className={`text-left space-y-1.5 p-3 rounded-lg transition-all border ${
                  step.done
                    ? 'border-transparent opacity-60 cursor-default'
                    : 'border-transparent hover:bg-gray-900/30 hover:border-gray-900 cursor-pointer'
                }`}
              >
                <span className="flex items-center gap-1.5 text-[10px] font-mono text-indigo-400 font-bold">
                  {step.done ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> : <Circle className="w-3.5 h-3.5 text-gray-600" />}
                  <span>{String(i + 1).padStart(2, '0')} / {step.done ? 'CONCLUÍDO' : 'PENDENTE'}</span>
                </span>
                ...
                >
                  ...
                  <div className="p-4 space-y-1 bg-gray-950">
                    <div className="flex justify-between items-center text-[9px] text-gray-500 font-mono">
                      <span>TEMPLATE: {templates.find((t) => t.id === project.templateId)?.name || 'Removido'}</span>
      ...
      <NewProjectWizard isOpen={isFlowOpen} flowType={flowType} onClose={() => setIsFlowOpen(false)} />
    </motion.div>
  );
};
