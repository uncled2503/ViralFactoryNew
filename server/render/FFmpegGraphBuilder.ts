...
import { FFmpegAnimationEngine } from './FFmpegAnimationEngine';
import { FFmpegAudioEngine } from './FFmpegAudioEngine';
import { FFmpegSubtitleEngine, SubtitleItem, SubtitleStyleConfig } from './FFmpegSubtitleEngine';
...
export class FFmpegGraphBuilder {
   ...
   * Completely independent of the React interface. Accepts infinite layers.
   */
  static build(
    layers: RenderLayer[],
    ...
    for (const layer of layers) {
      // Skip background layer since it's already mapped to input 0
    ...
    // Create a mapping of Layer ID -> Input Index for easier access during fragment generation
    ...
    for (const layer of layers) {
      ...
      if (type === 'video') {
        if (inputIndex !== undefined) {
          ...
          const volume = layer.data?.styles?.volume ?? 1.0;
          ...
          if (fit === 'cover') {
            // Apply scale-to-fit-cover logic dynamically inside the scale filter
        ...
        if (subtitleFilePath && typeof subtitleFilePath === 'string' && (subtitleFilePath.endsWith('.srt') || subtitleFilePath.endsWith('.ass'))) {
          // Native SRT / ASS subtitle filter
          ...
          // Check if any subtitle item has word-by-word timestamps
    ...
    if (audioFilterSegments.length > 0) {
      ...
      // Mix all collected audio streams utilizing FFmpegAudioEngine compileMix
      const mixedAudioLabel = '[mixed_audio]';
      const mixSegment = FFmpegAudioEngine.compileMix(ctx.audioLabels, mixedAudioLabel);
      
      // Since compileMix returns string starting with inputs, we can extract or append it directly
